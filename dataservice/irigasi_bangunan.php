...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid, ident, altitude, norec, k_bangunan, urut, nomenklatu, nama, kondisi, nama_di, lat, long,  public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM irigasi_bangunan");
	if($stmt->execute()){
		$id_count = 0;
