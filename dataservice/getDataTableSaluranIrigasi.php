...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid, nomenklatu, nama_di, urut, ident, COALESCE(desa, '-') AS desa, COALESCE(kecamatan, '-') AS kecamatan, k_saluran, nama, COALESCE(kondisi, '-') AS kondisi FROM irigasi_saluran ORDER BY nama, urut");
	if($stmt->execute()){
