...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid, nama_di, shape_leng, shape_area, skema, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM irigasi_daerah");
	if($stmt->execute()){
		$id_count = 0;
