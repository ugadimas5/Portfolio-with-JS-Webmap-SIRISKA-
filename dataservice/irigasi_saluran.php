...
try {
	...
	$stmt = $dbcon->prepare("SELECT gid, objectid, ident, k_saluran, urut, nomenklatu, nama, kondisi, nama_di, desa, kecamatan, public.ST_AsGeoJSON(public.ST_Transform((geom),4326),6) AS geojson FROM irigasi_saluran");
	if($stmt->execute()){
		$id_count = 0;
